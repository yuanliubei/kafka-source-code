...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    KafkaProducer(Map<String, Object> configs,
        ...
        try {
            ...
            this.time = time;
            //todo 获取clientId，如果没有就创建默认的clientId
            ...
            //todo 配置基本的元数据信息，如元数据信息中的Cluster信息
            if (metadata != null) {
                this.metadata = metadata;
            } else {
                this.metadata = new Metadata(retryBackoffMs, config.getLong(ProducerConfig.METADATA_MAX_AGE_CONFIG),
                    true, true, clusterResourceListeners);
                //todo 没有真正的获取元数据信息，只是将metadata中的cluster设置为Cluster.bootstrap(addresses)
                this.metadata.update(Cluster.bootstrap(addresses), Collections.emptySet(), time.milliseconds());
            }
            this.errors = this.metrics.sensor("errors");
            //todo 创建sender对象，是整个producer中的关键对象
            this.sender = newSender(logContext, kafkaClient, this.metadata);
            String ioThreadName = NETWORK_THREAD_PREFIX + " | " + clientId;
            //todo 创建sender守护线程，并启动
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, Metadata metadata) {
        //todo 每个网络连接可以忍受生产者发送到broker后没有相应的个数，默认值5，因为kafka发送有自动重试机制，所以如果要保证消息顺序，这个值应设置为1
        ...
        //todo 初始化NetworkClient，NetworkClient对象完成消息的发送、元数据信息的获取，本质是使用java的nio。
        /**
         * CONNECTIONS_MAX_IDLE_MS_CONFIG 网络连接空闲超时时间，如果网络空闲并超过了这个时间，那么网络会自动关闭
         * RECONNECT_BACKOFF_MS_CONFIG 重新获取host主机信息的间隔时间
         * SEND_BUFFER_CONFIG socket发送缓存
         * RECEIVE_BUFFER_CONFIG socket接受缓存
         */
        KafkaClient client = kafkaClient != null ? kafkaClient : new NetworkClient(
                ...
                throttleTimeSensor,
                logContext);
        //todo 消息发送失败重试次数，默认值是Integer.MAX_VALUE
        int retries = configureRetries(producerConfig, transactionManager != null, log);
        //todo 发送端acks，默认值是1
        /**
         * 0:发送到broker后就不管了，存在消息丢失风险
         * 1：发送到broker后，数据写入到leader分区后返回响应，也存在消息丢失风险
         * -1（all）：发送到broker后，数据写入到leader分区并且数据同步到所有follower分区后返回响应，效率较低
         */
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            //todo 获取元数据信息剩余时间，最大阻塞时间-获取元数据的时间
            ...
            //todo 将消息缓冲到accumulator中
            RecordAccumulator.RecordAppendResult result = accumulator.append(tp, timestamp, serializedKey,
                    serializedValue, headers, interceptCallback, remainingWaitMs);
            if (result.batchIsFull || result.newBatchCreated) {
                //todo 缓冲批次满了或者创建了新的批次，然后唤醒sender线程
    ...
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long maxWaitMs) throws InterruptedException {
        ...
        //todo 初始化KafkaProducer时已经设定了cluster信息
        Cluster cluster = metadata.fetch();

        if (cluster.invalidTopics().contains(topic))
            throw new InvalidTopicException(topic);
        //todo 把topic添加到元数据信息中
        metadata.add(topic);
        //todo topic所有分区数
        Integer partitionsCount = cluster.partitionCountForTopic(topic);
        // Return cached metadata if we have it, and if the record's partition is either undefined
        // or within the known partition range
        //todo 如果在元数据信息中的缓存中获取到分区信息，则直接返回
        ...
        do {
            ...
            //todo 如果获取到分区信息，则跳出do while循环，是`if (partitionsCount != null && (partition == null || partition < partitionsCount))`代码的反转含义
     ...
     * todo 获取消息发送的分区
     * 1.如果发送消息时指定了分区，则使用指定的分区
     * 2.如果发送消息没有指定分区，则使用分区器确认分区，实现是DefaultPartitioner
     */
