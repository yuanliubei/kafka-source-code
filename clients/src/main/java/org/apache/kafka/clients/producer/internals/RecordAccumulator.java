...
public final class RecordAccumulator {
    ...
    /**
     * todo 实现是CopyOnWriteMap，是kafka自己实现的，本质是利用【写时复制】一个不可变map来操作
     * 适用于读多写少的场景，只会对增删改加锁，查不会加锁
     * 数据只能保证最终一致性，因为在拷贝还没替换的时候，读取的数据是旧值
     * 另外因为每次每次put都需要拷贝整个map，如果数据量比较大，还是比较消耗内存的
     */
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            //todo 根据topicPartition获取到对应的队列，如果不存在就创建，底层是map结构，key为tp v为队列
            Deque<ProducerBatch> dq = getOrCreateDeque(tp);
            synchronized (dq) {
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                //todo 尝试添加到缓冲区中，因为此时还没有创建batch，所以会添加失败返回null
            ...
            /**
             * todo 在batch和消息大小中取一个最大值，用这个值去申请内存
             * 默认一个批次是16k
             */
            int size = Math.max(this.batchSize, AbstractRecords.estimateSizeInBytesUpperBound(maxUsableMagic, compression, key, value, headers));
            log.trace("Allocating a new {} byte message buffer for topic {} partition {}", size, tp.topic(), tp.partition());
            //todo 在bufferPool中申请内存
            buffer = free.allocate(size, maxTimeToBlock);
            synchronized (dq) {
                // Need to check if producer is closed again after grabbing the dequeue lock.
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                //todo 再次尝试添加到缓冲区，因为没有创建batch，所以还是会添加失败返回null
                RecordAppendResult appendResult = tryAppend(timestamp, key, value, headers, callback, dq);
                if (appendResult != null) {
     ...
     * todo 获取topicPartition对应的队列，队列中保存的批次
     */
    private Deque<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        Deque<ProducerBatch> d = this.batches.get(tp);
        if (d != null)
            //todo 如果map中已经存在，则直接返回
            return d;
        //todo 创建队列，并存储到map中
        d = new ArrayDeque<>();
        /**
         * todo putIfAbsent
         * 如果map中没有key对应的值，则val直接添加，返回值为null
         * 如果map中key已经有对应的值，则val还是原来的值，返回为原来的值
         */
